...
LinearDWA::LinearDWA(ros::NodeHandle &n_t) {
	...
	vstep = (max_trans_vel - min_trans_vel) / 8;
	wstep = (max_rot_vel - min_rot_vel) / 8;
	...
	gridsize = .0500;
	mapsize = 4.000; //mm
	noOfgrids = (mapsize / gridsize) + 0.5;
	dwa_map = new DWAMap(gridsize);
	n = n_t;
	command_pub = n.advertise<geometry_msgs::TwistStamped>("motor_command",
			100);
	usercommand_pub = n.advertise<geometry_msgs::TwistStamped>("user_command_logger",
				100);
	odom_sub = n.subscribe("odom", 1, &LinearDWA::odomCallback, this);
	interface_sub = n.subscribe("user_command", 3,
			&LinearDWA::usercommandCallback, this);
	occupancy_sub = n.subscribe("localmap", 1, &LinearDWA::occupancyCallback,
			this);
#ifdef DEBUG
...
void LinearDWA::odomCallback(const nav_msgs::Odometry& cmd) {
...
void LinearDWA::occupancyCallback(const nav_msgs::OccupancyGrid& og) {
	this->dwa_map->updateMap(og);

}

void LinearDWA::usercommandCallback(
...
//		this->humanInput.w = 0;
	...
	if (strcmp(cmd->header.frame_id.c_str(), "joystick")) {
	interface = Joystick;
	} else {
		interface = Button;
	}
	this->updateInputCommand(this->humanInput.v, this->humanInput.w, interface);
}

void LinearDWA::updateInputCommand(float v, float w, InterfaceType In) { // input command here is assumed to be normalised to [-1,1]
	inputDist.clear();
 ...
 */
float LinearDWA::computeHeading(Speed usercommand, Speed candidateSpeed) {
...
float LinearDWA::sqrt_approx(float z) {
...
float LinearDWA::computeClearance(Speed candidateSpeed) {
...
float LinearDWA::computeDistToNearestObstacle(Speed candidateSpeed) {
 ...
 */
vector<Pose> LinearDWA::getObstacles(Pose pose, vector<Pose> obstacles) {
 ...
 *
 */
float LinearDWA::computeVelocity(Speed candidateSpeed) {
 ...
 */
vector<Speed> LinearDWA::getAdmissibleVelocities(vector<Speed> admissibles, float upperbound=M_PI+1, float lowerbound=-M_PI-1) {
	admissibles.clear();
	...
	for (int i = 0; i < trajectories.size(); i++) {
		...
		if ((trajectories[i] > upperbound) || (trajectories[i] < lowerbound)) {
...
DynamicWindow LinearDWA::computeDynamicWindow(DynamicWindow dw) {
...
vector<Speed> LinearDWA::getResultantVelocities(
		vector<Speed> resultantVelocities, float upperbound=M_PI+1, float lowerbound=-M_PI-1 ) {
...
Speed LinearDWA::computeNextVelocity(Speed chosenSpeed) {
	...
	for (int j = 0; j < inputDist.size(); j++) {
		Speed input = inputDist[j];
		float ang = atan2(input.w, input.v);
		if (ang > upperbound) upperbound = ang;
		if (ang < lowerbound) lowerbound = ang;
	}
	upperbound += M_PI*45/180;
	lowerbound -= M_PI*45/180;
	ROS_INFO("upperbound: %f, lowerbound: %f", upperbound, lowerbound);
	resultantVelocities = getResultantVelocities(resultantVelocities, upperbound, lowerbound);
	...
	visualiseCostFn();
		...
		for (int i = 0; i < resultantVelocities.size(); i++) {
			...
			const Speed input = (const Speed) (getRealSpeed(inputDist[0]));
 ...
 */
void LinearDWA::visualiseCostFn() {
...
void LinearDWA::addCostFn(Speed speed, float G, float heading, float clearance,
...
void LinearDWA::run() {
	...
	while (ros::ok()) {
		...
		Speed userspeed = getRealSpeed(inputDist[0]);
